...
{
    public static async Task ExtractToDirectory(this ZipArchive source, string destinationDirectoryName, IProgress<int> prog, IEnumerable<string> blacklistedFiles, Action<ZipArchiveEntry, string>? preExtractHook = null, Action<ZipArchiveEntry, string>? afterExtractHook = null) {
            ...
            using (Stream zipstream = entry.Open()) {
                if (entry.IsSymlink()) {
                    ...
                    using (var file = new FileStream(FullPath, FileMode.Create, FileAccess.Write, FileShare.None)) {
                        preExtractHook?.Invoke(entry, FullNameFixed);
